# Remote Access Services Configuration - SECURE with SOPS
{ config, pkgs, ... }:

{
  ...
  # VNC Server Configuration - SECURE with password from SOPS
  systemd.user.services.x11vnc = {
    ...
    serviceConfig = {
      ...
      # VNC with password from SOPS secret file
      ExecStart = "${pkgs.x11vnc}/bin/x11vnc -display :10 -forever -shared -rfbport 5900 -passwdfile /run/secrets/vnc_password -noxdamage -wait 5";
